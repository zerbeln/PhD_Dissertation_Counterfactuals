...
cpdef calc_global(rover_history, poi_vals, poi_pos):
    ...
    cdef int poi_id, step_number, rover_id, observer_count, od_index
    ...
    for poi_id in range(number_pois):
        ...
        for step_number in range(total_steps):
            ...
            observer_distances = [0.0 for i in range(nrovers)]
...
cpdef calc_dpp(rover_history, poi_vals, poi_pos):
    ...
    cdef double[:] difference_reward = np.zeros(nrovers)
        ...
        for agent_id in range(nrovers):
            ...
            for poi_id in range(number_pois):
                ...
                for step_number in range(total_steps):
                    ...
                    if observer_count < coupling:  # Add counterfactual partners if needed
