...
class Scenario(BaseScenario):
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            agent.color = np.array(
                [0.45, 0.95, 0.45]) if not agent.adversary else np.array([0.95, 0.45, 0.45])
    ...
    def agent_reward(self, agent, world):
        ...
        for food in world.food:
            if self.is_collision(agent, food):
                rew += 2
        rew += 0.05 * \
            min([np.sqrt(np.sum(np.square(food.state.p_pos - agent.state.p_pos)))
                 for food in world.food])
    ...
    def adversary_reward(self, agent, world):
        ...
        if shape:
            rew -= 0.1 * \
                min([np.sqrt(np.sum(np.square(a.state.p_pos - agent.state.p_pos)))
                     for a in agents])
        if agent.collide:
            for ag in agents:
                for adv in adversaries:
