...
def init_rover_positions_fixed():  # Set rovers to fixed starting position
    ...
    for t in range(p.num_types):
        for rov_id in range(p.num_rovers):
            ...
            rover_positions[r_id, 0] = 0.5*p.world_size  # Rover X-Coordinate
            rover_positions[r_id, 1] = 0.5*p.world_size  # Rover Y-Coordinate
...
def init_poi_positions_four_corners():  # Statically set 4 POI (one in each corner)
    ...
    poi_positions[1, 0] = 0.0; poi_positions[1, 1] = (p.world_size - 1.0)  # Top left
    poi_positions[2, 0] = (p.world_size - 1.0); poi_positions[2, 1] = 0.0  # Bottom right
    poi_positions[3, 0] = (p.world_size - 1.0); poi_positions[3, 1] = (p.world_size - 1.0)  # Top right
