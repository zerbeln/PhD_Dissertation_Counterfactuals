...
# GLOBAL REWARDS ------------------------------------------------------------------------------------------------------
cpdef calc_global_reward(rover_history, rover_positions, poi_vals, poi_pos):
        ...
        for step_number in range(num_steps):
            ...
            for agent_id in range(n_rovers):
                ...
                if distance <= activation_dist: # Rover is in observation range
                    types_in_range.append(rover_positions[agent_id, 2])
...
# DIFFERENCE REWARDS -------------------------------------------------------------------------------------------------
cpdef calc_difference_reward(rover_history, rover_positions, poi_vals, poi_pos):
    cdef int n_rovers = p.num_rovers * p.num_types
    cdef int n_types = p.num_types
    cdef int n_pois = p.num_pois
    cdef double min_dist = p.min_distance
    ...
    g_reward = calc_global_reward(rover_history, rover_positions, poi_vals, poi_pos)
                ...
                for other_agent_id in range(n_rovers):
                    if agent_id != other_agent_id:
                        ...
                        if distance <= activation_dist:
                            types_in_range.append(rover_positions[other_agent_id, 2])
                    else:
...
# D++ REWARD ----------------------------------------------------------------------------------------------------------
cpdef calc_dpp_reward(rover_history, rover_positions, poi_vals, poi_pos):
    cdef int n_rovers = p.num_rovers * p.num_types
    cdef int n_types = p.num_types
    cdef int n_pois = p.num_pois
    ...
    # CALCULATE DIFFERENCE REWARD
    difference_reward = calc_difference_reward(rover_history, rover_positions, poi_vals, poi_pos)
    ...
    for counterfactual_count in range(coupling):
        ...
        for agent_id in range(n_rovers):
            ...
            self_dist = 0.0
            current_rtype = rover_positions[agent_id, 2]
...
# S-D++ REWARD --------------------------------------------------------------------------------------------------------
# cpdef calc_sdpp_reward(rover_history, rover_positions, poi_vals, poi_pos):
#     cdef int n_rovers = p.num_rovers * p.num_types
#     cdef int n_pois = p.num_pois
#     cdef double min_dist = p.min_distance
#     cdef int num_steps = p.num_steps + 1
